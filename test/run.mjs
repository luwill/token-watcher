...
console.log('\n[2b] 前端纯函数（lib/）');
{
  ...
  // ---- 数值格式化（独立手算对照）----
  // 单位一律用字母；不足五位数原样显示——把 1234 写成 "1K" 既丢精度又没省字符
  ok('fmt 不足五位原样', fmt(9999) === '9999' && fmt(0) === '0', `${fmt(9999)} ${fmt(0)}`);
  ok('fmt K 分档', fmt(12345) === '12.3K', fmt(12345));
  ok('fmt M 分档', fmt(1234567) === '1.23M', fmt(1234567));
  // B 的分档在 1e9，不是中文"亿"的 1e8——换单位不是逐字替换，档位边界也跟着变
  ok('fmt B 分档自 1e9 起', fmt(123456789) === '123.46M' && fmt(1234567890) === '1.23B',
    `${fmt(123456789)} ${fmt(1234567890)}`);
  // toFixed 会把尾数进位到 1000（999,999 → "1000.0K"），越出本档表示范围，须升一档
  ok('fmt 尾数进位时升档', fmt(999999) === '1.00M' && fmt(999999999) === '1.00B',
    `${fmt(999999)} ${fmt(999999999)}`);
  ok('fmtShort 尾数进位时升档', fmtShort(999999) === '1.0M' && fmtShort(999999999) === '1.0B',
    `${fmtShort(999999)} ${fmtShort(999999999)}`);
  ok('fmtShort 不足五位原样', fmtShort(1500) === '1500', fmtShort(1500));
  ok('fmtShort K/M/B 分档',
    fmtShort(12345) === '12K' && fmtShort(1500000) === '1.5M' && fmtShort(2500000000) === '2.5B',
    `${fmtShort(12345)} ${fmtShort(1500000)} ${fmtShort(2500000000)}`);
  // 只扫产生用户可见文案的那两个文件。format.js 的注释里保留了"亿=1e8 而 B=1e9"
  // 这句说明，扫源码全文会把注释一并算进去而误报。
  ok('用户可见文案不再残留中文数量单位',
    !/[亿万]/.test(read(join(ROOT, 'web/app.js')) + read(join(ROOT, 'web/index.html'))));
